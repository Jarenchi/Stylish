...
const Products: React.FC<ProductsProps> = ({ endpoint }) => {
  ...
  if (data.pages[0].data.length === 0) {
    return (
      <p className="py-48 text-center lg:py-[4.75rem]" data-testid="nodatatext">
        目前沒有相關資料，請更換其他關鍵字
      </p>
    );
  }
  return (
    <div className="max-w-[81.25rem] m-auto mb-12 lg:mb-24" data-testid="products-container">
      {data?.pages.map((page, i) => (
        // eslint-disable-next-line react/no-array-index-key
        <div className="flex flex-wrap justify-center mt-[4.375rem]" key={i}>
          {page.data.map((product: Product) => (
            <div className="ml-10" key={product.id} data-testid="product-cards">
